...
const char * const BITCOIN_CONF_FILENAME = "redebalocoin.conf";
...
#else
    const char* pszModule = "redebalocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Redebalocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Redebalocoin";
#else
    // Unix
    return pathRet / ".redebalocoin";
#endif
#endif
}
